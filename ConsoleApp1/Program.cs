    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new MovieContext())
            {
                ...
                Console.WriteLine();
                Console.WriteLine("Listar o Elenco de Star Wars V: The Empire Strikes Back (press any key...):");
                Console.ReadKey();
                var qA = from f in db.Movies
                         join actMovie in db.Characters on f.ID equals actMovie.MovieId
                         join actor in db.Actors on actMovie.ActorID equals actor.ActorID
                         where f.Title == "The Empire Strikes Back"
                         select new
                         {
                             Name = actor.Name,
                             Personagem = actMovie.Character
                         };

                Console.WriteLine("O Elenco é:");
                foreach (var a in qA)
                {
                    Console.WriteLine(a.Name + " atuando como " + a.Personagem);
                }

                //Atores que já fizeram um papel
                Console.WriteLine();
                Console.WriteLine("Atores que já fizeram o papel de James Bond (press any key...):");
                Console.ReadKey();
                var qB = from f in db.Actors
                         join actMovie in db.Characters on f.ActorID equals actMovie.ActorID
                         where actMovie.Character == "James Bond"
                         select f.Name;

                Console.WriteLine("Os atores são:");
                foreach (var a in qB.Distinct())
                {
                    Console.WriteLine(a);
                }

                //Qual ator fez mais vezes este papel
                Console.WriteLine();
                Console.WriteLine("Ator que desepenhou mais vezes o papel de James Bond (press any key...):");
                Console.ReadKey();
                var qC = from f in db.Actors
                         join actMovie in db.Characters on f.ActorID equals actMovie.ActorID
                         where actMovie.Character == "James Bond"
                         orderby f.Name.Count() descending
                         select f ;

                Console.WriteLine("O ator é:");
                Console.WriteLine(qC.First().Name);
                #endregion

                #region consultas do professor
                //Console.WriteLine();
